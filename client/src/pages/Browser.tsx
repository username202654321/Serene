...
import { ArrowLeft, ArrowRight, Bookmark, BookmarkCheck, Globe2, History, Home, List, Maximize2, Palette, Plus, RefreshCw, Search, ShieldCheck, Sparkles, X } from "lucide-react";
...
function NewTab({ onNavigate, wallpaper, setWallpaper }: { onNavigate: (value: string) => void; wallpaper: number; setWallpaper: (wallpaper: number) => void }) {
  const [input, setInput] = useState("");
  const submit = (event: FormEvent) => { event.preventDefault(); onNavigate(input); };
  return <div className={`new-tab-view wallpaper-${wallpaper}`}><div className="wallpaper-picker" aria-label="Wallpaper"><Palette size={14} /><div>{[0, 1, 2].map((item) => <button key={item} className={wallpaper === item ? "is-active" : ""} onClick={() => setWallpaper(item)} aria-label={`Use wallpaper ${item + 1}`} />)}</div></div><div className="new-tab-center"><div className="new-tab-mark"><Sparkles size={19} /></div><span className="eyebrow">Serene Browser</span><h1>Where to next?</h1><form className="new-tab-search" onSubmit={submit}><Globe2 size={17} /><input autoFocus value={input} onChange={(event) => setInput(event.target.value)} placeholder="Search or enter an address" /><kbd>Enter</kbd></form><div className="shortcut-row"><button onClick={() => onNavigate("https://developer.mozilla.org")}><span>MDN</span> MDN Web Docs</button><button onClick={() => onNavigate("https://wikipedia.org")}><span>W</span> Wikipedia</button><button onClick={() => onNavigate("https://duckduckgo.com")}><span>DDG</span> DuckDuckGo</button></div></div></div>;
}
